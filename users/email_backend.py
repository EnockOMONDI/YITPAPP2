"""
DEPRECATED: Custom email backend for YITP that handled SSL certificate issues with Gmail SMTP

This file is no longer used since YITP has migrated to Mailtrap API for email sending.
The Mailtrap API uses HTTPS REST calls instead of SMTP, so SSL certificate issues
with SMTP servers are no longer relevant.

Keeping this file for reference in case we need to revert to SMTP in the future.
"""

# DEPRECATED - No longer used with Mailtrap API
# import ssl
# import smtplib
